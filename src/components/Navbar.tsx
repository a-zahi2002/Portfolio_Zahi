        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
             <a href="#" className="group relative">
                <h1 className="text-xl font-bold text-gray-900 dark:text-white font-['Space_Grotesk'] tracking-widest">
                  A.ZAHI<span className="text-blue-500 dark:text-accent-cyan">.</span>
                </h1>
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-blue-500 dark:bg-accent-cyan transition-all duration-300 group-hover:w-full"></span>
             </a>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span className={`absolute -bottom-1 left-0 h-0.5 bg-blue-600 dark:bg-accent-cyan transition-all duration-300 ${activeSection === link.href.substring(1) ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
              </a>
            ))}
